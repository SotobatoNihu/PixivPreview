...
export class PopupUtil {
    ...
    popupImg(page, outerContainer, elem) {
        ...
        let l = offset.left;
        let w = 600 * (((page.pageType === pagetype.ranking) ? elem.clientWidth : elem.parentNode.parentNode.clientWidth) / page.siteImgMaxWidth) + 5;
    ...
    popupCaption(outerContainer, imageID: string) {
    ...
    popupManga(outerContainer, elem: any, count: string) {
    ...
    private imgsArrInit(outerContainer, mangaContainer, manga, primaryLink: string, l: number) {
        let margins = document.body.clientWidth - l * 600; //some blind frame adjusting
        if (margins > 0) outerContainer.style.left = margins / 2 - 10 + 'px';
    ...
    private getImgId(primaryLink: string) {
